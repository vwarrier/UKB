...
pheno  = bd[,c("f.eid", "f.25742.2.0", "f.31.0.0", "f.22009.0.1",
...
pheno2 = pheno[!is.na(pheno$f.25742.2.0),] #remove items that are na in the pheno
...
setnames(headmotion_pheno, 3, "t_headmotion")

write.table(headmotion_pheno, file = "~/UKB_v2/Plink_files/taskheadmotionphenobolt.txt", row.names = F, col.names = T, quote = F)
...
write.table(headmotion_covar, file = "~/UKB_v2/Plink_files/taskheadmotioncovarbolt.txt", row.names = F, col.names = T, quote = F)
...
write.table(remove3, file = "~/UKB_v2/Plink_files/taskheadmotionremovebolt.txt", row.names = F, col.names = T, quote = F)
